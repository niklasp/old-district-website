...
class TDVRGlitchPass extends Pass {

	constructor( dt_size = 1202 ) {
	...
	render( renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */ ) {
		...
		if ( this.curF % this.randX == 0 || this.goWild == true ) {
			...
			this.uniforms[ 'amount' ].value = Math.random() / 230;
		...
		} else if ( this.curF % this.randX < 30 ) {

			this.uniforms[ 'amount' ].value = Math.random() / 890;
			// this.uniforms[ 'angle' ].value = MathUtils.randFloat( - Math.PI, Math.PI );
			this.uniforms[ 'distortion_x' ].value = MathUtils.randFloat( 0, 0.1 );
			this.uniforms[ 'distortion_y' ].value = MathUtils.randFloat( 0, 0.1 );
	...
	generateTrigger() {
		this.randX = MathUtils.randInt( 320, 740 );
	}
