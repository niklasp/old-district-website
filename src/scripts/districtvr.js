...
export default function DistrictVR() {
  ...
  const updateArticle = (domEl, article) => {
    ...
    date.innerHTML = article.pubDate
      ? new Date(article.pubDate).toLocaleDateString("en-US", dateOptions)
      : "";
  ...
  const init = () => {
    if (postContainer) {
      fetch(
        "https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/thedistrictvr"
      )
        .then((resp) => resp.json())
